...
function view_report(){
    ...
    if(frm_date==''){
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                data.forEach(function(org_head) {
                    ...
                    $("#rpt_head").text('Billwise Sale Register '+date_format(frm_date)+' - '+date_format(td_date));
        ...
        $.ajax({
            type: "POST",
            url: "method/ajax.php",
            data: {pRpt_saler_fdate:frm_date,pRpt_saler_tdate:td_date,pRpt_sale_whare:whare_h},
            success: function (response) {
                // console.log(response);
                var data = JSON.parse(response);
                $("#sale_reg_tbl").empty();
                ...
                data.forEach(function(salereg) {
                    ...
                    if(salereg.is_print==1 && salereg.sales_Date==null && salereg.row_span!=null){
                        $("#sale_reg_tbl").append('<tr><td rowspan="'+ salereg.row_span +'"><a href="#" onclick="print_bill(this.id);" id="'+ salereg.Sales_Id +'">'+ salereg.bill_no +' </a><br><span class="font-weight-bold">'+  salereg.party_Name +'</span></td></tr>');
                    }
