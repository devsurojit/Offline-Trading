...
function load_party(){
    ...
    if(pParty_Type==null){
        simple_alert('Warning','Please Select Party Type First','warning');
    }
    else{
        $.ajax({
            type: "POST",
            ...
            success: function (response) {
                var data = JSON.parse(response);
                $("#pparty_Id").empty();
                $("#pparty_Id").append('<option value="" selected="" disabled="" >Select Party</option>');
                data.forEach(function(gl_data) {
                    $("#pparty_Id").append('<option value="'+ gl_data.Id +'" >'+ gl_data.Party_Name +'</option>');
                });
            }
        });
    }
...
function post_voucher(){
    ...
    if(pVouch_Date==''){
        ...
        $.ajax({
            type: "POST",
            url: "method/post_party_trans.php",
            data: {pVouch_Date:pVouch_Date,pRef_Vouch_No:pRef_Vouch_No,pNarration:pNarration,pVouch_Type:pVouch_Type,pPaid_Mode:pPaid_Mode,pLedg_Id:pLedg_Id,pBank_Id:pBank_Id,pSb_Id:pSb_Id,pAmounnt:pAmounnt,pParty_Type:pParty_Type,pBank_crg:pBank_crg},
            success: function (response) {
                console.log(response);
                var data = JSON.parse(response);
