		...
		<input type="hidden" id="tbl_data" name="tbl_data">
				<div class="form-row">
					<div class="form-group col-md-2 mb-2 is-focused bmd-form-group">
						<label class="bmd-label-floating">Chalan Date</label>
						<input type="date" class="form-control" id="chalan_date" value="<?php echo date('Y-m-d');  ?>" name="chalan_date" min="<?php echo $_SESSION['fin_frm_dt']  ?>" max="<?php echo date("Y-m-d");  ?>">
					</div>
				
					
					<div class="form-group col-md-2 mb-2 is-focused bmd-form-group">
					<label class="bmd-label-floating">Reference Chalan No</label>
					<input type="text" class="form-control" id="ref_cln_no" required="true" name="ref_cln_no" autocomplete="off">
					</div>
...
<div class="card-footer justify-content-center py-3">
	<button type="button" class="btn btn-primary" id="sav_btn" name="sav_btn" onclick="add_chalan();">Save</button>
</div>
...
?>
<script src="../assets/js/chalan_add.js"></script>
<?php
}
else{
